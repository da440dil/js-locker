...
describe('Redis Gateway', () => {
  beforeEach(() => {
    gateway = new RedisGateway(client);
  });
  ...
  it('should throw Error if key exists and has no TTL', async () => {
    ...
    await expect(gateway.set(key, value, ttl)).rejects.toThrow(new Error(RedisGateway.ErrKeyNameClash));
  });
  ...
  it('should throw Error if redis returns response of invalid type', async () => {
    const evalMock = jest.spyOn(client, 'eval');
    const err = new Error(RedisGateway.ErrInvalidResponse);
